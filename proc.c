...
//     // Using Forloop to find which process to run
//     acquire(&ptable.lock);
//     for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
//       if(p->state != RUNNABLE)
//         continue;
...
//   acquire(&ptable.lock);
//   cprintf("\n\n\n---------------------------------- Iteration---------------------------------------\n");
//   cprintf("Name \t\t Process ID \t State \t\t Tickets \t Currently Executing  \n");
//   for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
//   {
//     if (p->state == RUNNING)
//       cprintf("%s \t %d \t\t RUNNING  \t\t %d \t\t %d  \n ", p->name, p->pid, p->nice_pro1, p->in_use);
